...
#define CREATE_NEW_ALIAS(iter_hosts)                                         \
    ...
    void HHVM_METHOD(Aerospike, __construct, const Array& php_config, bool persistent_connection,
			 ...
			 const Variant& options)
    {
        ...
        PolicyManager       policy_manager(&write_policy, "write", &data->as_ref_p->as_p->config);
        ...
        if (!data->as_ref_p->as_p) {
            ...
            if (AEROSPIKE_OK == php_record_to_as_record(php_rec, rec, ttl, static_pool, error)) {
                if (AEROSPIKE_OK == policy_manager.set_policy(options, error)) {
                    ...
                    aerospike_key_put(data->as_ref_p->as_p, &error, &write_policy, &key, &rec);
    ...
    int64_t HHVM_METHOD(Aerospike, get, const Array& php_key, VRefParam php_rec, const Variant& options)
    {
        ...
        PolicyManager       policy_manager(&read_policy, "read", &data->as_ref_p->as_p->config);
        ...
        if (!data->as_ref_p->as_p) {
            ...
            if (AEROSPIKE_OK == policy_manager.set_policy(options, error)) {
                if (AEROSPIKE_OK == aerospike_key_get(data->as_ref_p->as_p, &error, &read_policy, &key, &rec_p)) {
            ...
            const Variant& options)
    {
        ...
        PolicyManager       policy_manager(&operate_policy, "operate", &data->as_ref_p->as_p->config);
        ...
        if (!data->as_ref_p->as_p) {
            ...
            if (AEROSPIKE_OK == php_operations_to_as_operations(php_operations, operations, static_pool, error)) {
                if (AEROSPIKE_OK == policy_manager.set_policy(options, error)) {
                    ...
                    aerospike_key_operate(data->as_ref_p->as_p, &error, &operate_policy, &key, &operations, &rec_p);
